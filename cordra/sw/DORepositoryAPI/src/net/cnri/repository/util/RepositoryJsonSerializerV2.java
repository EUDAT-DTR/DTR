...
import net.cnri.repository.util.RepositoryJsonSerializerV2.DOView.ElementView;
 ...
 * Also provides a method for directly loading a repository from a url that returns a JSON string.
 * 
...
public class RepositoryJsonSerializerV2 {
	...
	public static void loadRepositoryFromURL(Repository repository, String url) throws IOException, RepositoryException, CreationException {
		...
		List<DOView> objects = RepositoryJsonSerializerV2.fromJSON(json);
		RepositoryJsonSerializerV2.loadDOViewsIntoRepository(objects, repository);
	}
	
	public static List<DOView> getDOViewsFromURL(String url) throws IOException {
		String json = HttpClientUtil.getJSONFromURL(url);
		return RepositoryJsonSerializerV2.fromJSON(json);		
	}
...
//			public String getElementId() { return elementID; }
			...
			public byte[] getData() { return getDataStringAsBytes(); }
			
			public byte[] getDataStringAsBytes() {
				try {
				    if (dataAsString == null) return new byte[0];
					return dataAsString.getBytes("UTF-8");
			...
			public String getDataString() { return dataAsString; }
			...
			private void extractDataFrom(DataElement element, boolean includeData) throws IOException, RepositoryException {
				...
				if (includeData) {
					...
					//in = element.read();
					dataAsString = stringFromInputStream(in);
			...
			private static String stringFromInputStream(InputStream in) throws IOException {
				byte[] bytes = bytesFromInputStream(in);
